...
export class ModalPrestamo implements OnInit, OnChanges {
      ...
      if (this.edit && this.prestUpdate.state) {
        if (!this.form.get('state')) {
          this.form.addControl(
            'state',
            this.fb.control(this.prestUpdate.state)
          );
        } else {
  ...
  addBook(book: Book) {
    if (!this.booksSeleccionado.some((b) => b.codeBook === book.codeBook)) {
      ...
      this.authorSelected = this.listAuthores(book.author);
  ...
  removeBook(book: BookResponse) {
    this.booksSeleccionado = this.booksSeleccionado.filter(
      (b) => b.codeBook !== book.codeBook
    );
  }

  listAuthores(author: AuthorModel[]): string {
    if (!author || author.length === 0) {
      return 'Autor no disponible';
    }

    return author.map((a) => a.names + ' ' + a.lastname).join(', ');
  }
}
